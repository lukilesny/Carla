...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            arrayRGB = cv2.cvtColor(array, cv2.COLOR_BGR2RGB)
            display.show(arrayRGB)
...
def DetectLane(image):
    ...
    lowerWhite2 = np.array([22,76,240])
    upperWhite2 = np.array([35,194,255])
    maskWhite2 = cv2.inRange(image, lowerWhite2, upperWhite2)
    
    lowerWhite3 = np.array([20,78,255])
    upperWhite3 = np.array([43,164,255])
    maskWhite3 = cv2.inRange(image, lowerWhite3, upperWhite3)
    
    lowerWhite4 = np.array([18,90,237])
    upperWhite4 = np.array([34,152,255])
    maskWhite4 = cv2.inRange(image, lowerWhite4, upperWhite4)
    ...
    cntsW2 = cv2.findContours(maskWhite2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cntsW2 = cntsW2[0] if len(cntsW2) == 2 else cntsW2[1]
    
    cntsW3 = cv2.findContours(maskWhite3, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cntsW3 = cntsW3[0] if len(cntsW3) == 2 else cntsW3[1]
    
    cntsW4 = cv2.findContours(maskWhite4, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cntsW4 = cntsW4[0] if len(cntsW4) == 2 else cntsW4[1]
   ...
   # centers.sort(key = )
    ...
    x2 = end_point[0]
    y2 = end_point[1]
    while(0 < x1 < width and 0 < y1 < height and 0 < x2 < width and 0 < y2 < height):
        x1 = x1 - run
        y1 = y1 - rise
        x2 = x2 + run
        y2 = y2 + rise 
    ...
    if (xLane == 0):
        cv2.putText(copy, 'no lanes',(600, 600),  cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255) ,3)
    elif(xCar < xLane):
        cv2.putText(copy, 'turn right',(600, 600),  cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255) ,3)
    elif(np.abs(xCar - xLane) > 350 and xCar > xLane and xLane != 0):
        print(xLane)
        cv2.putText(copy, 'turn left',(600, 600),  cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255) ,3)
    else:
        cv2.putText(copy, 'keep straight',(600, 600),  cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255) ,3)
