...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        self._info_text += [
            ...
            '',
            '',
            'Detected sign:   ' + ('%d' % self.sign if self.sign>0 else 'None') ]
        if len(vehicles) > 1:
...
class CameraManager(object):
    ...
    def detect_signs(self,I):
        ...
        lower_red = np.array([165, 120, 90])
        upper_red = np.array([180, 255, 255])
        mask2 = cv2.inRange(hsv, lower_red, upper_red)
        # Generating the final mask to detect red color
        mask1 = mask1 + mask2
        circles = cv2.HoughCircles(mask1, cv2.HOUGH_GRADIENT, 1, 2000,
        ...
        try:
            ...
            for x,y,r in circles[0, :]:
                ...
                img = img[:,:,::-1]
                lp = self.clf.predict(HOG(img).reshape(1, -1))
                self.hud.sign = lp*30
                if(lp != 0):
